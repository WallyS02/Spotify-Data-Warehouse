...
CREATE TABLE TempPlaylistTrack(
    ID_P INT REFERENCES PLAYLIST,
    ID_T INT REFERENCES TRACK,
    PRIMARY KEY (ID_P, ID_T)
)
...
CREATE TABLE PLAYBACK(
    ...
    ID_P INT,
    ID_PT INT,
    FOREIGN KEY (ID_P, ID_PT) REFERENCES PLAYLIST_TRACK
)

CREATE TRIGGER DeletePlaybackForPlaylistTrack
ON PLAYLIST_TRACK
AFTER DELETE
AS
BEGIN
    UPDATE PLAYBACK SET ID_P = NULL, ID_PT = NULL
    WHERE ID_P IN (SELECT deleted.ID_P FROM deleted) AND ID_PT IN (SELECT deleted.ID_T FROM deleted);
END;

CREATE TRIGGER UpdatePlaybackForPlaylistTrack
ON PLAYLIST_TRACK
AFTER UPDATE
AS
BEGIN
    UPDATE pb
    SET pb.ID_P = i.ID_P, pb.ID_PT = i.ID_T
    FROM PLAYBACK pb
    INNER JOIN inserted i ON pb.ID_P = i.ID_P AND pb.ID_PT = i.ID_T;
END;
